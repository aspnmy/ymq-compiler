...
# Debug mode: set to True for verbose output, False for clean output
DEBUG_MODE = False
...
def ymq_stage1_analysis(imatrix_path, gguf_path):
    ...
    debug_print("Starting GGUF header parsing with GGUFReader...", file=sys.stderr)
    
    try:
        from gguf.gguf_reader import GGUFReader
        
        debug_print(f"Reading GGUF file: {gguf_path}", file=sys.stderr)
        ...
        debug_print(f"Found {len(tensor_element_counts)} tensors in GGUF file", file=sys.stderr)
        ...
        import traceback
        debug_print(f"Exception during tensor parsing: {e}", file=sys.stderr)
        debug_print(f"Traceback: {traceback.format_exc()}", file=sys.stderr)
    
    debug_print(f"Total tensors parsed: {len(tensor_element_counts)}", file=sys.stderr)
    ...
    # Read entire file to ensure we capture all imatrix data (fix: was limited to 16MB)
    try:
        with open(imatrix_path, 'rb') as f:
            imatrix_data = f.read()
    except Exception as e:
        print(f"ERROR: Failed to read imatrix file: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    for match in matches:
        ...
        # Read a larger chunk to ensure we capture all imatrix values (fix: was 1024 bytes)
        chunk_data = imatrix_data[start_offset:start_offset + 4096]
        floats = []
...
def ymq_stage2_assign_targets(data, input_target, high_target, mid_target, low_target, copy_target, tiny_target, default_target, small_threshold_gb, tiny_threshold_gb, has_mtp):
    ...
    debug_print(f"Python received has_mtp='{has_mtp}'", file=sys.stderr)
    ...
    debug_print(f"Found nextn tensors in GGUF: {nextn_found}", file=sys.stderr)
    ...
    if has_mtp.lower() == "true" or (has_mtp.lower() == "auto" and nextn_found):
        debug_print(f"Scanning for FFN layers missing imatrix data...", file=sys.stderr)
        ffn_layer_nums = set()
        for tensor_name in tensor_element_counts:
        ...
        debug_print(f"MTP detection - ffn_layer_nums={sorted(ffn_layer_nums)}", file=sys.stderr)
        debug_print(f"MTP detection - max_layer={max_layer}, effective_max_layer={effective_max_layer}", file=sys.stderr)
        for layer_num in ffn_layer_nums:
            if layer_num > effective_max_layer:
                mtp_layers.add(layer_num)
        debug_print(f"MTP layers detected: {sorted(mtp_layers)}", file=sys.stderr)
    ...
    # Target rank reflects quality (higher rank = higher quality = more bits per weight)
    # Used to determine which target to assign when multiple targets could apply to the same layer
    # Use BPW_MAP values as ranks so that higher-quality targets (more BPW) get higher ranks
    target_rank = {target: BPW_MAP.get(target, 0) for target in [INPUT_TARGET, HIGH_TARGET, COPY_TARGET, TINY_TARGET, MID_TARGET, LOW_TARGET, DEFAULT_TARGET, MTP_TARGET]}
    ...
    if all_layer_scores:
        ...
        # Fix: Handle edge cases where n is too small
        if n >= 4 and len(significant_scores) >= 4:
            ...
            # Fix: Handle small n cases safely
            if n > 0:
                tier_thresholds = [
                    sorted_scores[n // 4] if n >= 4 else sorted_scores[0],
                    sorted_scores[n // 2] if n >= 2 else sorted_scores[0],
                    sorted_scores[3 * n // 4] if n >= 4 else sorted_scores[-1]
                ]
            else:
                tier_thresholds = [0.0, 0.0, 0.0]
        
        tier_thresholds = sorted(set(tier_thresholds))
        if len(tier_thresholds) < 3:
            # Fix: Handle case where all thresholds are the same
            if n > 0:
...
def main():
    if len(sys.argv) < 11:
        print("Usage: python3 ymq_compiler.py <imatrix_path> <gguf_path> <input_target> <high_target> <mid_target> <low_target> <copy_target> <tiny_target> <default_target> <small_threshold_gb> <tiny_threshold_gb> <has_mtp>", file=sys.stderr)
        sys.exit(1)
